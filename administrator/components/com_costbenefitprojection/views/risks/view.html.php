...
class CostbenefitprojectionViewRisks extends JView
{
	protected $items;
	...
	{		
		...
		JToolBarHelper::title(JText::_('COM_COSTBENEFITPROJECTION_RISKS_TITLE'), 'risks');

		if ($canDo->get('core.create')) {
			JToolBarHelper::addNew('risk.add');
		}
		if ($canDo->get('core.edit')) {
			JToolBarHelper::editList('risk.edit');
		}

		if ($canDo->get('core.edit.state')) {
			JToolBarHelper::divider();

			JToolBarHelper::publishList('risks.publish');
			JToolBarHelper::unpublishList('risks.unpublish');
	
			JToolBarHelper::divider();
	
			JToolBarHelper::archiveList('risks.archive');
			
			if ($canDo->get('core.admin')) {
				JToolBarHelper::checkin('risks.checkin');
			}
		}
		...
		if ($canDo->get('core.delete')) {
			...
			if ($this->state->get('filter.published') == '-2') {
				JToolBarHelper::deleteList('', 'risks.delete');
			} else {
				JToolBarHelper::trash('risks.trash');
			}
		}
