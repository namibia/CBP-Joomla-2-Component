...
class CostbenefitprojectionViewDiseasecategories extends JView
{
	protected $items;
	...
	{	
		...
		JToolBarHelper::title(JText::_('COM_COSTBENEFITPROJECTION_DISEASECATEGORIES_TITLE'), 'diseasecategories');
		
		if ($canDo->get('core.create')) {
			JToolBarHelper::addNew('diseasecategory.add');
		}
		if ($canDo->get('core.edit')) {
			JToolBarHelper::editList('diseasecategory.edit');
		}

		if ($canDo->get('core.edit.state')) {
			JToolBarHelper::divider();

			JToolBarHelper::publishList('diseasecategories.publish');
			JToolBarHelper::unpublishList('diseasecategories.unpublish');
	
			JToolBarHelper::divider();
	
			JToolBarHelper::archiveList('diseasecategories.archive');
			
			if ($canDo->get('core.admin')) {
				JToolBarHelper::checkin('diseasecategories.checkin');
			}
		}
		...
		if ($canDo->get('core.delete')) {
			...
			if ($this->state->get('filter.published') == '-2') {
				JToolBarHelper::deleteList('', 'diseasecategories.delete');
			} else {
				JToolBarHelper::trash('diseasecategories.trash');
			}
		}
