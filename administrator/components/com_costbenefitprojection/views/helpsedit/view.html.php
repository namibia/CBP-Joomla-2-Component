...
class CostbenefitprojectionViewHelpsedit extends JView
{
	protected $items;
	...
	{	
		...
		JToolBarHelper::title(JText::_('COM_COSTBENEFITPROJECTION_HELPSEDIT_EDITOR_TITLE'), 'helps');
		
		if ($canDo->get('core.create')) {
			// JToolBarHelper::addNew('helpedit.add');
		}
		if ($canDo->get('core.edit')) {
			JToolBarHelper::editList('helpedit.edit');
		}

		if ($canDo->get('core.edit.state')) {
			JToolBarHelper::divider();

			JToolBarHelper::publishList('helpsedit.publish');
			JToolBarHelper::unpublishList('helpsedit.unpublish');
	
			JToolBarHelper::divider();
	
			JToolBarHelper::archiveList('helpsedit.archive');
			
			if ($canDo->get('core.admin')) {
				JToolBarHelper::checkin('helpsedit.checkin');
			}
		}
		...
		if ($canDo->get('core.delete')) {
			...
			if ($this->state->get('filter.published') == '-2') {
				JToolBarHelper::deleteList('', 'helpsedit.delete');
			} else {
				JToolBarHelper::trash('helpsedit.trash');
			}
		}
