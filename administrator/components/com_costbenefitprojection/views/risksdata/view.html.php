...
class CostbenefitprojectionViewRisksdata extends JView
{
	protected $items;
	...
	{	
		...
		JToolBarHelper::title(JText::_('COM_COSTBENEFITPROJECTION_RISKSDATA_TITLE'), 'risksdata');

		if ($canDo->get('core.create')) {
			JToolBarHelper::addNew('riskdata.add');
		}
		if ($canDo->get('core.edit')) {
			JToolBarHelper::editList('riskdata.edit');
		}

		if ($canDo->get('core.edit.state')) {
			JToolBarHelper::divider();

			JToolBarHelper::publishList('risksdata.publish');
			JToolBarHelper::unpublishList('risksdata.unpublish');
	
			JToolBarHelper::divider();
	
			JToolBarHelper::archiveList('risksdata.archive');
		
			if ($this->user['type'] == 'admin') {
				JToolBarHelper::checkin('risksdata.checkin');
			}
		}
		...
		if ($canDo->get('core.delete')) {
			...
			if ($this->state->get('filter.published') == '-2') {
				JToolBarHelper::deleteList('', 'risksdata.delete');
			} else {
				JToolBarHelper::trash('risksdata.trash');
			}
		}
