...
class CostbenefitprojectionModelRisks extends JModelList
{
	...
	public function getItems()
	{
		...
		foreach ($items as &$item) {
			$item->url = 'index.php?option=com_costbenefitprojection&amp;task=risk.edit&amp;risk_id=' . $item->risk_id;
			$item->risk_description = $this->truncate($item->risk_description);
	...
	public function getListQuery()
	{
		...
		$query->from('#__costbenefitprojection_risk AS t');
		...
		if (!empty($search)) {
			...
			$field_searches =
				"(t.risk_name LIKE {$search} OR " .
				"t.risk_description LIKE {$search})";
	...
	protected function checkInNow()
	{
		...
		if ($time){
			...
			// Check table
			$query->update($db->quoteName('#__costbenefitprojection_risk'))->set($fields)->where($conditions); 
