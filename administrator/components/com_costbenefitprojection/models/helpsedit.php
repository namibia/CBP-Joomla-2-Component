...
class CostbenefitprojectionModelHelpsedit extends JModelList
{
	...
	public function getItems()
	{
		...
		if ($serviceProvider || $country_user){
		...
		foreach ($items as &$item) {
			$item->url = 'index.php?option=com_costbenefitprojection&amp;task=helpedit.edit&amp;help_id=' . $item->help_id;
	...
	public function getListQuery()
	{
		...
		$query->select('#__costbenefitprojection_helps.*, u.name');
		$query->from('#__costbenefitprojection_helps');

		$query->join('LEFT', '#__users AS u ON u.id = #__costbenefitprojection_helps.checked_out');
		...
		if (!empty($search)) {
			...
			$field_searches =
				"(help_description LIKE '{$search}' OR " .
				"help_name LIKE '{$search}')";
	...
	protected function checkInNow()
	{
		...
		if ($time){
			...
			// Check table
			$query->update($db->quoteName('#__costbenefitprojection_helps'))->set($fields)->where($conditions); 
